...
export default function Navbar() {
  ...
  const [navbarOpen, setNavbarOpen] = useState(false);
   ...
   const MENU = [
    ...
    return (
    <nav className="border-gray-200 px-2 sm:px-4 py-2.5 bg-gray-900 opacity-80">
      <div className={`container flex ${navbarOpen && 'flex-wrap'} justify-between items-center mx-auto sticky`}>
        <ActiveLink href="/">
          <a className="flex">
            <Image
            width="139px"
        ...
        <button
         ...
         data-collapse-toggle="mobile-menu" type="button" className="inline-flex items-center p-2 ml-3 text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="mobile-menu-2" aria-expanded="false">
          <span className="sr-only">Open main menu {console.log("user state", userState)}</span>
        ...
        <div className={`${navbarOpen ? '' : 'hidden'} w-full md:block md:w-auto" id="mobile-menu`}>
          <ul className="flex flex-col justify-end mt-4 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium">
            ...
            <li key={index}>
              <ActiveLink href={url}>
              <a className="block py-2 pr-4 pl-3 text-gray-200 border-b border-gray-100 hover:bg-gray-50 md:hover:bg-transparent md:border-0 md:hover:text-orange-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700">{title}</a>
              </ActiveLink>
            </li>
            ))}
            <li>
              <ActiveLink href={`/${userState ? 'dashboard' : '/api/auth/login'}`}>
              <a className="block py-2 pr-4 pl-3 text-gray-200 border-b border-gray-100 hover:bg-gray-50 md:hover:bg-transparent md:border-0 md:hover:text-orange-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700">{`${userState ? 'Dashboard' : 'Login'}`}</a>
              </ActiveLink>
            </li>
          </ul>
        </div>
